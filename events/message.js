...
let lavender = db.fetch(`lavender.botbakim`)
  ...
  if (cmd) {
  if(message.author.id !== ayarlar.sahip && lavender) return message.channel.send(`<a:unlem:724733762541453384> **Bot Şuan Bakımdadır, Lütfen Sahibim Açana Kadar Beklemede Kalınız.**`)
    if (perms < cmd.conf.permLevel) return;
    cmd.run(client, message, params, perms);
  }
};
